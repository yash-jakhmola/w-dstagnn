...
class modwt(nn.Module):
    ...
    def forward(self, x):
        level = 2 #int(np.log(x.shape[-1]))=2 
        ...
        for j in range(level):
            w = self.circular_convolve_d(h_t, v_j_1, j + 1)
            v_j_1 = self.circular_convolve_d(g_t, v_j_1, j + 1)
            wavecoeff.append(w)
        wavecoeff.append(v_j_1)
        return torch.stack(wavecoeff)

    def circular_convolve_d(self, h_t, v_j_1, j):
        '''
        ...
        w_j = torch.zeros_like(v_j_1 , device=self.DEVICE)
        l = np.arange(L)

        for t in range(N):
            index = np.mod(t - 2 ** (j - 1) * l, N)
            w_j[:,:,:, t] = v_j_1[:,:,:, index[0]]*h_t[0] + v_j_1[:,:,:, index[1]]*h_t[1]
        return w_j

class imodwt(nn.Module):
    def __init__(self, DEVICE):
        super(imodwt, self).__init__()
        self.DEVICE = DEVICE
    ...
    def forward(self, w):
        ...
        for jp in range(level):
            j = level - jp - 1
            v_j = self.circular_convolve_s(h_t, g_t, w[j], v_j, j + 1)
        return v_j

    def circular_convolve_s(self, h_t, g_t, w_j, v_j, j):
        '''
        ...
        v_j_1 = torch.zeros_like(v_j, device=self.DEVICE)
        l = np.arange(L)

        for t in range(N):
            index = np.mod(t + 2 ** (j - 1) * l, N)
            v_j_1[:,:,:, t] = w_j[:,:,:, index[0]]*h_t[0] + w_j[:,:,:, index[1]]*h_t[1]
            v_j_1[:,:,:, t] = v_j_1[:,:,:, t] + v_j[:,:,:, index[0]]*g_t[0] + v_j[:,:,:, index[1]]*g_t[1]
        return v_j_1
...
class DSTAGNN_block(nn.Module):
    ...
    def __init__(self, DEVICE, num_of_d, in_channels, K, nb_chev_filter, nb_time_filter, time_strides,
        ...
        self.wavetrans = modwt(DEVICE)
        self.invwavetrans = imodwt(DEVICE)

        self.adj_pa = torch.FloatTensor(adj_pa).to(DEVICE)

        self.pre_conv = nn.Conv2d(num_of_timesteps, d_model, kernel_size=(1, num_of_d))

        self.EmbedT_1 = Embedding(DEVICE, num_of_timesteps, num_of_vertices, num_of_d, 'T')
        self.EmbedT_2 = Embedding(DEVICE, num_of_timesteps, num_of_vertices, num_of_d, 'T')
        self.EmbedT_s = Embedding(DEVICE, num_of_timesteps, num_of_vertices, num_of_d, 'T')
        self.EmbedS = Embedding(DEVICE, num_of_vertices, d_model, num_of_d, 'S')

        self.TAt_1 = MultiHeadAttention(DEVICE, num_of_vertices, d_k, d_v, n_heads, num_of_d)
        self.TAt_2 = MultiHeadAttention(DEVICE, num_of_vertices, d_k, d_v, n_heads, num_of_d)
        self.TAt_s = MultiHeadAttention(DEVICE, num_of_vertices, d_k, d_v, n_heads, num_of_d)
    ...
    def forward(self, x, res_att):
        ...
        x_wt = self.wavetrans(x)
        if isinstance(res_att, int):
            res_att_wt = np.array([res_att, res_att, res_att])
        else:
            res_att_wt = self.wavetrans(res_att)

        # TAT_1
        if num_of_features == 1:
            TEmx_1 = self.EmbedT_1(x_wt[0], batch_size)  # B,F,T,N
        else:
            TEmx_1 = x_wt[0].permute(0, 2, 3, 1)
        TATout_1, re_At_1 = self.TAt_1(TEmx_1, TEmx_1, TEmx_1, None, res_att_wt[0])  # B,F,T,N; B,F,Ht,T,T

        # TAT_2
        if num_of_features == 1:
            TEmx_2 = self.EmbedT_2(x_wt[1], batch_size)  # B,F,T,N
        else:
            TEmx_2 = x_wt[1].permute(0, 2, 3, 1)
        TATout_2, re_At_2 = self.TAt_2(TEmx_2, TEmx_2, TEmx_2, None, res_att_wt[1])  # B,F,T,N; B,F,Ht,T,T

        # TAT_s
        if num_of_features == 1:
            TEmx_s = self.EmbedT_s(x_wt[2], batch_size)  # B,F,T,N
        else:
            TEmx_s = x_wt[2].permute(0, 2, 3, 1)
        TATout_s, re_At_s = self.TAt_s(TEmx_s, TEmx_s, TEmx_s, None, res_att_wt[2])  # B,F,T,N; B,F,Ht,T,T
 
        TATout = self.invwavetrans([TATout_1, TATout_2, TATout_s])
        re_At = self.invwavetrans([re_At_1, re_At_2, re_At_s])
